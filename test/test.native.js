...
tape( 'the function returns the mode of a Cauchy distribution', opts, function test( t ) {
	var expected;
	var gamma;
	var x0;
	var y;
	...
	for ( i = 0; i < x0.length; i++ ) {
		y = mode( x0[i], gamma[i] );
		t.strictEqual( y, expected[i], 'x0 :'+x0[i]+', gamma: '+gamma[i]+', y: '+y+', expected: '+expected[i] );
	}
	t.end();
});
